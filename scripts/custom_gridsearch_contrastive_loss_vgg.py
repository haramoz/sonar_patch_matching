...
def create_base_network_vgg(input_shape,conv2d_filters,kernel_sizes,initialization,layers,dense_filter1,dense_filter2,dense_filter3,dropout1,dropout2,dropout3,use_bn):
    ...
    if layers == "three":
        x = Dense(dense_filter3, kernel_initializer=keras.initializers.he_normal(seed=random_seed),activation='relu')(x)
        if use_bn:        
            x = BatchNormalization()(x)
...
def fit_model(data,conv2d_filters,kernel_sizes,initialization,layers,dense_filter1,dense_filter2,dense_filter3,dropout1,dropout2,dropout3,use_bn,batch_size,opt,lr,epochs,loss):
    input_shape = (1,96,96)
    num_classes = 2
...
def process_fit(config):
    ...
    for i in range(trials):
        acc,auc = fit_model(data,conv2d_filters,kernel_sizes,initialization,layers,dense_filter1,dense_filter2,dense_filter3,dropout1,dropout2,dropout3,use_bn,batch_size,optimizer,lr,epochs,loss)
        accs.append(acc)
        aucs.append(auc)
