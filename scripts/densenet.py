"""Credits https://github.com/tdeboissiere/DeepLearningImplementations/blob/master/DenseNet/densenet.py"""
...
def DenseNet_Siamese(nb_classes, img_dim, depth, nb_dense_block, growth_rate,
    ...
    x = Dense(64, kernel_initializer=he_normal(), kernel_regularizer=l2(weight_decay),bias_regularizer=l2(weight_decay))(x)
    x = Activation('relu')(x)
    x = BatchNormalization()(x)
    

    #x = Dense(1, activation = 'sigmoid',name='fc', kernel_regularizer=l2(weight_decay),bias_regularizer=l2(weight_decay))(x)
    """x = Dense(nb_classes,
              activation='softmax',
              kernel_regularizer=l2(weight_decay),
              bias_regularizer=l2(weight_decay))(x)"""

    densenet = Model(inputs=[model_input], outputs=[x], name="DenseNet_Siamese")
...
def DenseNet_original(nb_classes, img_dim, depth, nb_dense_block, growth_rate,
             nb_filter, dropout_rate=None, weight_decay=1E-4):
    """ Build the DenseNet model
